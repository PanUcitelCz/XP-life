<script lang="ts">
    ...
    type Props = {
        categoryName?: string;
        activities?: Activity[]; // Pole aktivit nebo undefined
        totalPoints?: number;
        onAddXP: (activityId: number) => void;
    };

    const { categoryName, activities = [], totalPoints, onAddXP, ...restProps }: Props = $props(); // Defaultní hodnota prázdného pole
            ...
            {#each activities as activity (activity.id)}
                <div class="activity-row">
                    ...
                    <div class="activity-actions">
                        ...
                        {:else}
                            <button onclick={() => handleAddXP(activity.id)}>Add XP</button>
                        {/if}
                    </div>
                </div>
            {/each}
